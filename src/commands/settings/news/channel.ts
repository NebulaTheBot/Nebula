...
export default class Channel {
  ...
  constructor(db?: QuickDB<any>) {
    ...
    this.data = new SlashCommandSubcommandBuilder()
      ...
      .addRoleOption(option => option
        .setName("role")
        .setDescription("The role to ping when news are sent."));
  }
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    if (!interaction.memberPermissions.has(PermissionsBitField.Flags.ManageGuild)) return await interaction.followUp({
      embeds: [errorEmbed("You need **Manage Server** permissions to add news.")],
    });

    if (channel.type !== ChannelType.GuildText && channel.type !== ChannelType.GuildAnnouncement) return await interaction.followUp({
      embeds: [errorEmbed("The channel must be a text channel.")] 
    });

    await newsTable.set(`${guild.id}.channel`, { channelId: channel.id, roleId: role?.id ?? "" });
