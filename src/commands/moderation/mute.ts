...
export default class Mute {
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    if (!member.permissions.has(PermissionsBitField.Flags.MuteMembers)) return await interaction.reply({
      embeds: [errorEmbed("You need the **Mute Members** permission to execute this command.")]
    });

    if (target === member) return await interaction.reply({ embeds: [errorEmbed("You can't mute yourself.")] });
    ...
    if (!ms(duration) || ms(duration) > ms("28d")) return await interaction.reply({
    ...
    if (dmChannel) await dmChannel.send({ embeds: [embed.setTitle("🤐 • You were muted").setColor(genColor(0))] });
