...
export default class Edit {
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    interaction.client.once("interactionCreate", async interaction => {
      ...
      sendSubscribedNews(guild, { ...newNews, title: `Updated: ${newNews.title}` } as News).catch(
      ...
      if (subscribedNewsChannel.channelId) {
        ...
        const newsChannel = (await guild.channels
          .fetch(subscribedNewsChannel?.channelId ?? "")
      ...
      const embed = new EmbedBuilder().setTitle("✅ • News edited!").setColor(genColor(100));
